...
int main() {
    ...
    for (int i = 0; i < MAX_HEIGHT; i++) {
        for (int j = 0; j < MAX_WIDTH; j++) {
            ...
            // Apply invert filter (Software result)
            sw_result[i][j][0] = 255 - red;
            sw_result[i][j][1] = 255 - green;
            sw_result[i][j][2] = 255 - blue;

            // Output the inverted values for each pixel
            printf("Pixel (%d, %d) Inverted: R = %d, G = %d, B = %d\n", i, j, sw_result[i][j][0], sw_result[i][j][1], sw_result[i][j][2]);
        }
    }
    ...
    for (int k = 0; k < MAX_HEIGHT; k++) {
        for (int l = 0; l < MAX_WIDTH; l++) {
            ...
            // Call the hardware-accelerated invert function
            unsigned char newRed, newGreen, newBlue;
            invert_filter(red, green, blue, &newRed, &newGreen, &newBlue);

            // Store hardware results
            hw_result[k][l][0] = newRed;
            hw_result[k][l][1] = newGreen;
            hw_result[k][l][2] = newBlue;
        }
    }
#endif
